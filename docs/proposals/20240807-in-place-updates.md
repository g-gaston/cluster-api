...
If the totality of the required changes cannot be covered by the defined extensions, Cluster API will by default fall back to the current behavior (rolling update).

## Motivation

Cluster API by default performs rollouts by creating a new machine and deleting the old one. At the current alpha stage of the in-place updates feature, the default rolling update is always enabled and cannot be turned off.
...
If external update extensions can not cover the totality of the desired changes, users WILL defer to Cluster API’s default, immutable rollouts. This is important for a couple of reasons:
...
Initially, this feature will be implemented without affecting the current functionality of Cluster API. It will follow Kubernetes' feature gate mechanism and be contained within the experimental package. This means that any changes in behavior are controlled by the feature gate `InPlaceUpdates`, which must be enabled by users for the new in-place updates workflow to be available. It is disabled unless explicitly configured.
...
If after iterating over all external updaters a change could not be handled by any of them, CAPI will determine the desired state cannot be reached through external updaters. In this case, it will fallback to the rolling update strategy, replacing machines as needed. 
